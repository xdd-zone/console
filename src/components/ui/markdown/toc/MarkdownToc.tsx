  ...
  return (
    <aside className={clsx('sticky top-0 hidden h-fit lg:block', className)} aria-label="Table of contents">
      <div className="p-3 text-sm">
        ...
        <div className="mt-3 border-t border-[var(--color-border-secondary)]/10 pt-3 dark:border-white/10">
          <TocProgress progress={progress} />
          <button
            type="button"
            onClick={scrollToTop}
            className="mt-3 flex cursor-pointer items-center gap-1 text-xs text-[var(--color-text-secondary)]/60 hover:text-[var(--color-text-primary)] dark:text-white/60 dark:hover:text-white"
            aria-label="返回顶部"
          >
            <ArrowUp size={14} />
            返回顶部
