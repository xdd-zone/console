...
export const defaultMarkdownTheme: MarkdownTheme = {
  ...
  blockquote: {
    blockquote: 'my-4 rounded-lg border-l-4 border-primary bg-bg-secondary p-4 italic text-text',
  },
  ...
  heading: {
    ...
    h1: 'mt-6 mb-3 text-3xl font-bold tracking-tight text-text',
    h2: 'mt-5 mb-2 text-2xl font-semibold tracking-tight text-text',
    h3: 'mt-4 mb-2 text-xl font-semibold text-text',
    h4: 'mt-3 mb-1 text-lg font-semibold text-text',
    h5: 'mt-3 mb-1 text-base font-semibold text-text',
    h6: 'mt-3 mb-1 text-sm font-semibold uppercase tracking-wide text-text',
  },
  // 分隔线：页面语义分隔
  hr: {
  ...
  // 行内文本样式：代码、删除线、斜体、加粗
  inline: {
    code: 'rounded bg-bg-secondary px-1.5 py-0.5 font-mono text-sm text-text',
    del: 'text-text-muted line-through decoration-rosewater',
    em: 'italic text-text',
  ...
  table: {
    ...
    tbody: '',
    td: 'border-b border p-2 text-left align-top text-text',
    th: 'border-b border p-2 text-left font-medium text-text',
    thead: 'bg-bg-secondary',
    tr: 'odd:bg-transparent even:bg-bg-tertiary',
  },
}
